package examples.ZSChain

import akka.actor.ActorRef
import examples.commons.{SimpleBoxTransaction, SimpleBoxTransactionMemPool, SimpleBoxTransactionSerializer}
import examples.ZSChain.api.http.{DebugApiRoute, StatsApiRoute, WalletApiRoute}
import examples.ZSChain.blocks._
import examples.ZSChain.history.{HybridHistory, HybridSyncInfo, HybridSyncInfoMessageSpec}
import examples.ZSChain.mining._
import examples.ZSChain.wallet.SimpleBoxTransactionGeneratorRef
...
class HybridApp(val settingsFilename: String) extends Application {
  ...
  import examples.ZSChain.wallet.SimpleBoxTransactionGenerator.ReceivableMessages.StartGeneration
