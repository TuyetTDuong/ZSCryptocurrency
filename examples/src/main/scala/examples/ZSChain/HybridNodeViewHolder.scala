...
import examples.ZSChain.history.{HybridHistory, HybridSyncInfo}
import examples.ZSChain.mining.{HybridMiningSettings, HybridSettings}
...
object HybridNodeViewHolder extends ScorexLogging with ScorexEncoding {
  ...
  (HybridHistory, HBoxStoredState, HBoxWallet, SimpleBoxTransactionMemPool) = {
    ...
    val posGenesis = PosBlock.create(powGenesis.id, powGenesis.id, 0, genesisTxs, genesisBox, attachment, genesisAccountPriv)
