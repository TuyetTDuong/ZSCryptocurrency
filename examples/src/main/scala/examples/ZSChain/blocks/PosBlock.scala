package examples.ZSChain.blocks
...
case class PosBlock(override val parentId: BlockId, //previous block
                    randBeacon: BlockId, //PoW block
...
object PosBlockSerializer extends ScorexSerializer[PosBlock] with ScorexEncoding {
  ...
  override def serialize(b: PosBlock, w: Writer): Unit = {
    ...
    w.putBytes(idToBytes(b.randBeacon))
  ...
  override def parse(r: Reader): PosBlock = {
    ...
    val randBeacon = bytesToId(r.getBytes(BlockIdLength))
    ...
    PosBlock(parentId, randBeacon, timestamp, txs, box, attachment, signature)
  }
}
...
object PosBlock extends ScorexEncoding {
  ...
  implicit val posBlockEncoder: Encoder[PosBlock] = (psb: PosBlock) => {
    Map(
      ...
      "randBeacon" -> encoder.encodeId(psb.randBeacon).asJson,
  ...
  def create(parentId: BlockId,
    ...
    val unsigned = PosBlock(parentId, randBeacon, timestamp, txs, box, attachment, Signature25519(Signature @@ Array[Byte]()))
