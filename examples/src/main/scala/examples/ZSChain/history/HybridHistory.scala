package examples.ZSChain.history


import java.io.File

import examples.commons.{FileLogger, SimpleBoxTransaction}
import examples.ZSChain.blocks._
import examples.ZSChain.mining.HybridMiningSettings
import examples.ZSChain.validation.{DifficultyBlockValidator, ParentBlockValidator, SemanticBlockValidator}
...
class HybridHistory(val storage: HistoryStorage,
  ...
  private def calcDifficultiesForNewBlock(posBlock: PosBlock): (BigInt, BigInt) = {
    ...
    println("CALCULATING DIFFICULTY")
    ...
    if (powHeight > DifficultyRecalcPeriod && powHeight % DifficultyRecalcPeriod == 0) {
      ...
      // TODO: review me .get and asInstanceOf
      @SuppressWarnings(Array("org.wartremover.warts.OptionPartial"))
      //val lastPos = modifierById(posBlock.parentId).get.asInstanceOf[PowBlock]
      val lastPow = modifierById(posBlock.randBeacon).get.asInstanceOf[PowBlock]
      ...
      val realTimePowHead = lastPow.brothers.headOption match {
        case Some(block) => block.timestamp
        case None       => 0
      }
      val realTimePowLast = lastPow.brothers.lastOption match {
        case Some(block) => block.timestamp
        case None       => 0
      }
      val realTimePow = realTimePowLast.toString.toLong - realTimePowHead.toString.toLong
      val realTimePoS = posBlock.timestamp - lastPow.timestamp
      ...
      val newPowDiffUnlimited = (oldPowDifficulty * (expectedTime / (realTime + realTimePoS))).max(BigInt(1L))
      ...
      val newPosDiff = (oldPosDifficulty * (expectedTime / (realTime + realTimePow))).max(BigInt(1L))
      println("NEW POW DIFF "+newPowDiff)
      println("NEW POS DIFF "+newPosDiff)
      println("NUMBER OF BROTHER "+brothersCount)
...
object HybridHistory extends ScorexLogging {
  val DifficultyRecalcPeriod: Int = 2
