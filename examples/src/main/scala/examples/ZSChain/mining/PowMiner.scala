...
import examples.ZSChain.blocks.{HybridBlock, PowBlock, PowBlockHeader, PowBlockSerializer}
import examples.ZSChain.history.HybridHistory
import examples.ZSChain.state.HBoxStoredState
import examples.ZSChain.util.Cancellable
  ...
  override def receive: Receive = {
    ...
    case pmi: PowMiningInfo =>
      ...
      if (!cancellableOpt.forall(_.status.isCancelled)) {
        ...
        println("Best POW ID: "+bestPowBlock.header.id)
